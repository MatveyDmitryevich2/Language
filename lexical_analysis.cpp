...
static void Process_bracket(size_t* iterating_through_array, size_t* i, Identifiers* identifiers, Types side_bracket);
...
Identifiers* Lexical_analysis(Info_buffer* info_buffer)
{
    ...
    for (size_t i = 0, iterating_through_array = 0; i < info_buffer->size; )
    {
        ...
        if (info_buffer->buffer[i] == LEFT_BRACKET_)
        {
            DPRINTF("LEFT_BRACKET_: %c\n", info_buffer->buffer[i]);
            Process_bracket(&iterating_through_array, &i, identifiers + iterating_through_array, Types_LEFT_BRACKET);
        }

        else if (info_buffer->buffer[i] == RIGHT_BRACKET_)
        {
                              ...
                              Identifiers* identifiers, Info_buffer* info_buffer)
{
    ...
    else
    {
        ...
        {
            identifiers->type = Types_VARIABLE;
